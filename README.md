...
Make sure that everything is under control version and change version number if needed. After that you could run to publish it.
...
**major** - Big changes on the internals - change of `Gatsby` version, reworking the code, redesign and any big change.

**minor** - Small changes - build changes, code refactoring, SEO, small component tweaks, new pages.

**patch** - New posts - for every new post increment it.

Because the **patch** is changing how [`semantic versioning`](https://semver.org/) works and it's define. Don't use `npm version` command to change the version cause it will reset it to zero when using the `major` and `minor` command.