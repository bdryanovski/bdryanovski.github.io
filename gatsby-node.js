...
exports.createSchemaCustomization = ({ actions, schema }, themeOptions) => {
  ...
  const {basePath} = withDefaults(config)
...
exports.onCreateNode = ({ node, actions, getNode, createNodeId, createContentDigest }, themeOptions) => {
  ...
  const {postsPath, pagesPath} = withDefaults(config)

  // @IMPORTANT - this is the line that ignore mdx files marked as draft to be build at all
  // Ignore drafts
  if (node.frontmatter && node.frontmatter.draft && node.frontmatter.draft === true) {
    return
  }
...
exports.createPages = async ({ actions, graphql, reporter }, themeOptions) => {
  ...
  const { basePath, blogPath, tagsPath, formatString, postsPrefix } = withDefaults(config)
